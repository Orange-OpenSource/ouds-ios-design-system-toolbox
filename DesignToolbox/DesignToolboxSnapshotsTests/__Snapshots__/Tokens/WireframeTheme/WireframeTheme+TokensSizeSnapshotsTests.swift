...
final class WireframeThemeTokensSizeSnapshotsTests: TokensSizeSnapshotsTestsTestCase {
    ...
    /// This function tests all size tokens in the `WireframeTheme` with the `light` color scheme.
    ...
    /// This function tests all size tokens in the `WireframeTheme` with the `dark` color scheme.
    /// It iterates through all `NamedSize` cases, rendering each illustration in a `UIHostingController`
    /// and captures a snapshot. The snapshot is saved with a name indicating the elevation, theme, and color scheme.
