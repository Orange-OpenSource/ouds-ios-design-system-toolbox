...
/// Tests the UI rendering of each **border token** using reference images for `WireframeTheme`
final class WireframeThemeTokensBorderSnapshotsTests: TokensBorderSnapshotsTestsTestCase {
    ...
    /// This function tests all border tokens in the `WireframeTheme` with the `light` color schemes.
    ...
    @MainActor func testAllBorderWireframeThemeLight() {
    ...
    /// This function tests all border tokens in the `WireframeTheme` with  the `dark` color schemes.
    /// It iterates through all `NamedSize` cases, rendering each illustration in a `UIHostingController`
    /// and captures a snapshot. Each snapshot is saved with a name that indicates the spacing type, theme, and color scheme.
    @MainActor func testAllBorderWireframeThemeDark() {
