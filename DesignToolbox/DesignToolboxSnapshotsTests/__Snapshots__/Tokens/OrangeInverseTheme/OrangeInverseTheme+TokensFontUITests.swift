...
final class OrangeInverseThemeTokensFontUITests: TokensFontUITestsTestCase {
    ...
    /// This function tests all font tokens in the `OrangeInverseTheme` with the `light` color scheme.
    ...
    @MainActor func testAllFontsOrangeInverseThemeLight() {
        let interfaceStyle = UIUserInterfaceStyle.light
        testAllFonts(for: theme, in: interfaceStyle)
    }

    /// This function tests all font tokens in the `OrangeInverseTheme` with the `dark` color scheme.
    /// It iterates through all `NamedFont` cases, rendering each illustration in a `UIHostingController`
    /// and captures a snapshot. The snapshot is saved with a name indicating the font, theme, color scheme horizontalSizeClass.
    @MainActor func testAllFontsOrangeInverseThemeDark() {
