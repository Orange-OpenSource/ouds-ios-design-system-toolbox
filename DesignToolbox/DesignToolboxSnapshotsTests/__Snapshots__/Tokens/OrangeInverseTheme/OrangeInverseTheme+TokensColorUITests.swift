...
final class OrangeInverseThemeTokensColorUITests: TokensColorUITestsTestCase {
    ...
    /// This function tests all color tokens in the `OrangeInverseTheme` with the `light` color schemes.
    ...
    @MainActor func testAllColorsOrangeInverseThemeLight() {
    ...
    /// This function tests all color tokens in the `OrangeInverseTheme` with the `dark` color schemes.
    /// It iterates through all sections of `NamedColor`, rendering each illustration in a `UIHostingController`,
    /// and captures a snapshot for each color. The snapshots are saved with names indicating the color, theme, and color scheme.
    @MainActor func testAllColorsOrangeInverseThemeDark() {
        ...
        /*
         ...
         - OrangeInverse Theme, dark mode --> colorRepositoryOpacityBlackHighest
         - OrangeInverse Theme, dark mode --> colorSurfaceStatusNeutralEmphasized
         */
